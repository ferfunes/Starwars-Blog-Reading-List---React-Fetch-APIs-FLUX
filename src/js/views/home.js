	...
	render() {
		let peopleCard = this.state.character.map((item, index) => {
			return (
				<Card
					key={index}
					imgUrl="https://lumiere-a.akamaihd.net/v1/images/first-order_8ae2955f.jpeg?region=0%2C0%2C1560%2C878&width=768"
					prop1={item.name}
					prop1Title="Name : "
					prop2={item.height}
					prop2Title="Height : "
		...
		let planetCard = this.state.planet.map((item, index) => {
			return (
				<Card
					key={index}
					imgUrl="https://lumiere-a.akamaihd.net/v1/images/Death-Star-II_b5760154.jpeg?region=0%2C0%2C2160%2C1215&width=768"
					prop1={item.name}
					prop1Title="Name : "
					prop2={item.terrain}
					prop2Title="Terrain : "
